...
We'll use this table for all examples.
...
Without a `WHERE` clause, **every row will be updated**, so be careful.
...
⚠️ This removes both the table and all its data.
...
genui{"computing_relational_databases_learning_block_staging":{"type_id":"SQL_GROUP_BY"}}
...
genui{"computing_relational_databases_learning_block_staging":{"type_id":"SQL_JOIN"}}
...
genui{"computing_relational_databases_learning_block_staging":{"type_id":"SQL_TRANSACTION_COMMIT_ROLLBACK"}}
...
| `TRUNCATE TABLE` | Remove all rows while keeping the table |
| `COUNT()`        | Count rows                              |
...
| `ROLLBACK`       | Undo a transaction before committing    |

## Tips for Beginners

* Always use a `WHERE` clause with `UPDATE` and `DELETE` unless you intentionally want to affect every row.
* Practice with a small sample database before working with real data.
* Write SQL keywords in uppercase for readability, even though SQL is generally case-insensitive.
* Learn `SELECT`, `WHERE`, `ORDER BY`, and `JOIN` first—they are the commands you'll use most often.
